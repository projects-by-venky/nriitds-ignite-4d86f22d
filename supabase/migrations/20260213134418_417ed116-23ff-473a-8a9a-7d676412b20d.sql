...
-- 1. Drop and recreate events_public view to mask contact info for non-admin/faculty users
...
-- 2. Drop and recreate research_projects_public view to mask contact_email for non-admin/faculty users
DROP VIEW IF EXISTS public.research_projects_public CASCADE;

CREATE VIEW public.research_projects_public AS
SELECT
    id,
    category,
    contributor_type,
...
-- 3. Restrict syllabus_reviews SELECT to admin/faculty or the teacher who created it
DROP POLICY IF EXISTS "Authenticated users can view syllabus reviews" ON public.syllabus_reviews;

CREATE POLICY "Admin/faculty can view all syllabus reviews"
ON public.syllabus_reviews
FOR SELECT
TO authenticated
USING (public.is_admin_or_faculty(auth.uid()));

CREATE POLICY "Teachers can view their own syllabus reviews"
ON public.syllabus_reviews
FOR SELECT
TO authenticated
USING (email = (SELECT profiles.email FROM profiles WHERE profiles.user_id = auth.uid()));
